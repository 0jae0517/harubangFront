...
const RequestDetailPage: React.FC = () => {
    ...
    if (!requestDetails) {
        return <div className="p-8 text-center">요청서 정보를 찾을 수 없습니다.</div>;
    }
            ...
            <div className="bg-harubang-sky min-h-full py-12">
                <div className="container mx-auto px-6 max-w-3xl">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                        ...
                        <p className="text-harubang-ink-light mt-2">{requestDetails.date}에 등록된 신청서입니다.</p>
                    </motion.div>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-3 border-b pb-4">
                                ...
                                <div>
                                    ...
                                    <p className="font-semibold text-lg">{requestDetails.customerName}</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="flex items-center gap-3"><Building size={24} className="text-harubang-blue"/><div><p className="text-sm text-gray-500">희망 매물</p><p className="font-semibold text-lg">{requestDetails.propertyType}</p></div></div>
                                <div className="flex items-center gap-3"><Home size={24} className="text-harubang-blue"/><div><p className="text-sm text-gray-500">거래 종류</p><p className="font-semibold text-lg">{requestDetails.transactionType}</p></div></div>
                                <div className="flex items-center gap-3"><Banknote size={24} className="text-harubang-blue"/><div><p className="text-sm text-gray-500">예산</p><p className="font-semibold text-lg">{requestDetails.deposit} {requestDetails.rent ? `/ ${requestDetails.rent}`:''}</p></div></div>
                            </div>
                            <div>
                                <p className="text-sm text-gray-500 mb-2">상세 요청사항</p>
                                <p className="bg-gray-50 p-4 rounded-md text-gray-700 leading-relaxed">{requestDetails.details}</p>
                            </div>
                        </div>
