        ...
        {myRequests.length > 0 ? (
          <ul className="divide-y divide-gray-200">
            {myRequests.map((req, index) => (
              ...
              >
                <div>
                  <p className="font-semibold text-lg text-harubang-ink">
                    {req.title}
                  </p>
                ...
                <div className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <p
                      className={`font-bold ${req.status.includes("제안 도착") ? "text-green-600" : "text-gray-500"}`}
                    >
                      {req.status}
                    </p>
                  </div>
                  {req.status.includes("제안 도착") && (
                    <Link
                      to={`/proposals/${req.id}`}
                      className="mt-2 inline-flex items-center gap-1.5 text-sm text-white bg-harubang-blue font-semibold hover:bg-harubang-blue-dark py-2 px-4 rounded-lg transition-colors"
                    >
                      <Eye size={14} /> 제안 보기
                    </Link>
                  )}
          ...
          <div className="text-center text-gray-500 py-8">
            ...
            <Link
              to="/apply"
              className="mt-4 inline-block text-harubang-blue font-semibold hover:underline"
            >
              첫 신청서 작성하러 가기
            </Link>
          </div>
