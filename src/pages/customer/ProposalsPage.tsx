...
const ProposalsPage: React.FC = () => {
  ...
  const proposals = state.proposals.filter(
    (p) => p.requestId === Number(requestId),
  );
      ...
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <p className="text-harubang-ink-light mt-2">
            신청서 ID: {requestId}에 대해 중개사님들이 보내주신 매물 제안입니다.
          </p>
        </motion.div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <h2 className="text-xl font-bold my-2 text-harubang-blue">
                    {property.name}
                  </h2>
                  <div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-sm text-harubang-ink-light my-3">
                    ...
                    <span className="flex items-center gap-1.5">
                      <Banknote size={14} /> {property.deal} | {property.price}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Square size={14} /> {property.area}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Home size={14} /> 방 {property.rooms}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Bath size={14} /> 욕실 {property.baths}
                    </span>
                  </div>
