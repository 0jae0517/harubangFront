    ...
    <div className="flex h-screen bg-harubang-sky/50">
      ...
      <aside className="w-64 bg-white border-r flex flex-col flex-shrink-0">
        ...
        <nav className="flex-grow p-4 space-y-2">
          <NavLink to="/admin/dashboard" className={getNavStyle} end>
            <LayoutDashboard size={20} /> 대시보드
          </NavLink>
          <NavLink to="/admin/users" className={getNavStyle}>
            <Users size={20} /> 회원 관리
          </NavLink>
          <NavLink to="/admin/requests" className={getNavStyle}>
            <FileText size={20} /> 신청서 관리
          </NavLink>
          <NavLink to="/admin/properties" className={getNavStyle}>
            <Building size={20} /> 매물 관리
          </NavLink>
        </nav>
        <div className="p-4 border-t">
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-red-50 hover:text-red-600"
          >
            <LogOut size={20} /> 로그아웃
          </button>
        </div>
