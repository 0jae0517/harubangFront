...
type Action =
  | {
      type: "ADD_REQUEST";
      payload: Omit<Request, "id" | "status" | "hasNew" | "date" | "title">;
    }
    ...
    case "ADD_PROPOSAL":
      ...
      const updatedRequests = state.requests.map((req) => {
        if (req.id === action.payload.requestId) {
          const proposalCount =
            state.proposals.filter((p) => p.requestId === req.id).length + 1;
          return {
            ...req,
            status: `제안 도착 (${proposalCount}건)`,
            hasNew: true,
          };
        }
        return req;
